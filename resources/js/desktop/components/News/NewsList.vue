<template>
    <div class="section mb-5">
        ...
        <carousel :items="2" :nav="false" :dots="true" class="news-block">
            <div @click="show(index+1)" v-for="(item, index) in filteredNews">
                <news :index="index+1" :image="item.image"/>
            </div>

        </carousel>

        <news-modals :active="active_index" :city="city"/>
    </div>
</template>
<script>
    import News from '../News/News'
    import carousel from 'vue-owl-carousel'
    import NewsModals from "../News/NewsModals";
    ...
    export default {
        ...
        computed: {
            filteredNews() {
                ...
                let tmp = this.news.filter(item => item.location === this.city);
